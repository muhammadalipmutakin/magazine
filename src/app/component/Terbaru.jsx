...
const Terbaru = () => {
  ...
  useEffect(() => {
    const fetchLatestBlogs = async () => {
      ...
      setError(null);
      try {
        const res = await fetch("/api/blogs?latest=true&limit=3");
        if (!res.ok) {
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center">
        ...
        <Link href="/terbaru">
          <p className="badge-orange rounded-full p-[8px_18px] bg-[#FFECE1] font-bold text-sm leading-[21px] text-[#FF6B18] w-fit cursor-pointer">
            UP TO DATE
          </p>
        </Link>
      </div>
